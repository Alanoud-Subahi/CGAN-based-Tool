...
import pandas as pd
from cgan_modelF import CGAN  # Ensure this matches your actual file name
...
def main():
    ...
    if st.session_state.df is not None:
        ...
        # User inputs for integer columns without decimals (optional)
        int_columns = st.sidebar.multiselect("Select numerical features to preserve as integers without decimals (optional)", numerical_cols)
        ...
        if st.sidebar.button("Generate Synthetic Data"):
            if not class_column or not categorical_cols or not numerical_cols:
                ...
                cgan = CGAN(categorical_cols, numerical_cols, class_column, latent_dim=100, int_columns=int_columns if int_columns else None)
                df_preprocessed = cgan.preprocess_data(st.session_state.df, numerical_cols, categorical_cols)
